...
type mockTeamRepo struct {
	createTeamFn     func(*models.Team, int64, string) (error, int64)
	getTeamByIDFn    func(int64) (*models.Team, error)
	getTeamsByUserFn func(int64) ([]*models.Team, error)
	userExistsFn     func(int64) (bool, error)
	getTeamRoleFn    func(int64, int64) (string, error)
	updateTeamFn     func(*models.Team) error
	deleteTeamFn     func(int64) error
	addMemberFn      func(int64, int64, string) error
	removeMemberFn   func(int64, int64) error
	updateRoleFn     func(int64, int64, string) error
...
func (m *mockTeamRepo) CreateTeam(team *models.Team, userID int64, userRole string) (error, int64) {
	m.lastCreatedTeam = team
	m.lastCreateUserID = userID
	m.lastCreateUserRole = userRole
	if m.createTeamFn != nil {
		return m.createTeamFn(team, userID, userRole)
	}
	return nil, 7
}

func (m *mockTeamRepo) GetTeamByID(id int64) (*models.Team, error) {
...
func (m *mockTeamRepo) GetTeamsByUserID(userID int64) ([]*models.Team, error) {
	if m.getTeamsByUserFn != nil {
		return m.getTeamsByUserFn(userID)
	}
	return []*models.Team{{ID: 1, Name: "team"}}, nil
}

func (m *mockTeamRepo) UpdateTeam(team *models.Team) error {
	m.lastUpdatedTeam = team
	if m.updateTeamFn != nil {
		return m.updateTeamFn(team)
	}
	return nil
}

func (m *mockTeamRepo) DeleteTeam(id int64) error {
...
func (m *mockTeamRepo) UserExists(userID int64) (bool, error) {
...
func (m *mockTeamRepo) GetTeamRole(teamID int64, userID int64) (string, error) {
	if m.getTeamRoleFn != nil {
		return m.getTeamRoleFn(teamID, userID)
	}
	return "main_manager", nil
}

func (m *mockTeamRepo) AddMember(teamID int64, userID int64, role string) error {
	m.lastAddTeamID = teamID
	m.lastAddUserID = userID
	m.lastAddRole = role
	if m.addMemberFn != nil {
		return m.addMemberFn(teamID, userID, role)
	}
	return nil
}

func (m *mockTeamRepo) RemoveMember(teamID int64, userID int64) error {
...
func (m *mockTeamRepo) UpdateMemberRole(teamID int64, userID int64, newRole string) error {
	m.lastUpdateRoleTeam = teamID
	m.lastUpdateRoleUser = userID
	m.lastUpdateRoleName = newRole
	if m.updateRoleFn != nil {
		return m.updateRoleFn(teamID, userID, newRole)
	}
	return nil
}
...
func TestTeamService_CreateTeam(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setup: func(repo *mockTeamRepo) {
				repo.createTeamFn = func(*models.Team, int64, string) (error, int64) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			team, err := service.CreateTeam(tc.teamName, 11, tc.role)
			if tc.wantErr {
				if err == nil {
...
func TestTeamService_UpdateAndDeletePermissions(t *testing.T) {
	repo := &mockTeamRepo{
		getTeamByIDFn: func(id int64) (*models.Team, error) {
			return &models.Team{ID: id, Name: "Old", CreatedAt: time.Now(), UpdatedAt: time.Now()}, nil
		},
		getTeamRoleFn: func(teamID, userID int64) (string, error) {
			if userID == 1 {
				return "member", nil
			}
			return "main_manager", nil
	...
	if _, err := service.UpdateTeam(5, "New Name", 1); !customErrors.IsErrorType(err, customErrors.ErrTypeForbidden) {
		t.Fatalf("expected forbidden update for member, got %v", err)
	}

	updated, err := service.UpdateTeam(5, "New Name", 2)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if err := service.DeleteTeam(5, 1); !customErrors.IsErrorType(err, customErrors.ErrTypeForbidden) {
		t.Fatalf("expected forbidden delete for member, got %v", err)
	}

	if err := service.DeleteTeam(5, 2); err != nil {
...
func TestTeamService_MemberManagement(t *testing.T) {
	repo := &mockTeamRepo{
		...
		userExistsFn: func(userID int64) (bool, error) {
			if userID == 999 {
				return false, nil
			}
			return true, nil
	...
	if err := service.AddMemberToTeam(10, 20, 1); !customErrors.IsErrorType(err, customErrors.ErrTypeForbidden) {
		t.Fatalf("expected forbidden add by member, got %v", err)
	}

	if err := service.AddMemberToTeam(10, 20, 2); !customErrors.IsErrorType(err, customErrors.ErrTypeConflict) {
		t.Fatalf("expected conflict on duplicate member, got %v", err)
	}

	if err := service.AddMemberToTeam(10, 999, 2); !customErrors.IsErrorType(err, customErrors.ErrTypeNotFound) {
		t.Fatalf("expected not found for missing target user, got %v", err)
	}

	if err := service.RemoveMemberFromTeam(10, 20, 1); !customErrors.IsErrorType(err, customErrors.ErrTypeForbidden) {
		t.Fatalf("expected forbidden remove by member, got %v", err)
	}

	if err := service.RemoveMemberFromTeam(10, 20, 2); err != nil {
	...
	if err := service.UpdateMemberRole(10, 20, "invalid", 2); !customErrors.IsErrorType(err, customErrors.ErrTypeValidation) {
		t.Fatalf("expected validation error for invalid role, got %v", err)
	}

	if err := service.UpdateMemberRole(10, 20, "manager", 2); err != nil {
