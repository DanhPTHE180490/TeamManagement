...
func (s *teamService) CreateTeam(ctx context.Context, name string, userID int64, userRole string) (*models.Team, error) {
	...
	if len(name) > 100 {
		return nil, errors.NewValidationError("team name", "cannot exceed 100 characters")
	}

	// Validate permission
	if userRole == "member" {
		log.Printf("Unauthorized team creation attempt by member user %d", userID)
		return nil, errors.NewForbiddenError("members cannot create teams")
	}
	...
	if err != nil {
		...
		return nil, errors.NewInternalError("Failed to create team", err)
	}
...
func (s *teamService) UpdateTeam(ctx context.Context, id int64, name string, requesterID int64) (*models.Team, error) {
	...
	if err != nil {
		if errors.IsErrorType(err, errors.ErrTypeNotFound) {
			...
			return nil, errors.NewForbiddenError("you are not a member of this team")
		}
		log.Printf("Database error checking team role for user %d in team %d: %v", requesterID, id, err)
		return nil, errors.NewInternalError("Failed to verify permissions", err)
	}

	if requesterRole == "member" {
		log.Printf("Team member %d attempted to update team %d", requesterID, id)
		return nil, errors.NewForbiddenError("only managers can update team information")
	}
...
func (s *teamService) DeleteTeam(ctx context.Context, id int64, requesterID int64) error {
	...
	if err != nil {
		...
		log.Printf("Database error retrieving team %d: %v", id, err)
		return errors.NewInternalError("Failed to retrieve team", err)
	}
	...
	if err != nil {
		if errors.IsErrorType(err, errors.ErrTypeNotFound) {
			log.Printf("User %d is not a member of team %d", requesterID, id)
			return errors.NewForbiddenError("you are not a member of this team")
		}
		log.Printf("Database error checking team role for user %d in team %d: %v", requesterID, id, err)
		return errors.NewInternalError("Failed to verify permissions", err)
	}
...
func (s *teamService) AddMemberToTeam(ctx context.Context, teamID int64, targetID int64, requesterID int64) error {
	...
	if targetID <= 0 {
		return errors.NewValidationError("user ID", "must be greater than 0")
	}
	if targetID == requesterID {
		return errors.NewValidationError("user ID", "cannot add yourself to a team (you should already be a member)")
	}
...
func (s *teamService) UpdateMemberRole(ctx context.Context, teamID int64, targetID int64, newRole string, requesterID int64) error {
	...
	if newRole != "member" && newRole != "manager" && newRole != "main_manager" {
		return errors.NewValidationError("role", "must be 'member', 'manager', or 'main_manager'")
	}
	...
	if requesterRole != "main_manager" {
		...
		return errors.NewForbiddenError("only the main manager can update member roles")
	}
	...
	if err != nil {
		...
		return errors.NewInternalError("Failed to verify member status", err)
	}
